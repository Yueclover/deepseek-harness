...
没有该导出的包会被静默跳过。当源码开发 loader 把包解析到配置树的 Node 依赖查找范围之外时，本插件会定位最近的所属包清单，并导入其已构建的 `./typert` 产物。解析结论与已导入的 manifest 会在整个进程生命周期内缓存，因此新增 `./typert` 导出后必须重启。已挂载配置项对应的产物格式错误时，激活会大声失败；之后才发生的失败按包记录日志，不会阻止无关包完成注册。无法从配置树或源码开发 loader 解析、或缺少该导出的显式 `packages` 条目会大声失败并指名该包。
